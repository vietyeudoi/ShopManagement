...
namespace Sieu_Thi_Mini.Areas.Customer.Controllers
{
    ...
    public class OrderController : Controller
    {
        ...
        [HttpPost]
        public IActionResult Checkout(Sieu_Thi_Mini.Models.Customer customer, string OrderNote, string payment)
        {
            var cart = HttpContext.Session.GetObject<List<CartItem>>(CART_KEY);
            ...
            var order = new Order
            {
                ...
                Status = string.IsNullOrEmpty(OrderNote) ? "Chờ xác nhận" : $"Ghi chú: {OrderNote}"
            };
            ...
            var paymentEntry = new Payment
            {
                ...
                PaymentMethod = payment, // Nhận giá trị "COD" hoặc "Bank" từ Form
                PaymentDate = DateTime.Now,
                PaymentStatus = (payment == "COD") ? "Chưa thanh toán" : "Chờ xử lý"
            };
